...
print_summary() {
    ...
    $ENABLE_CLAUDE && echo "    /today                    # Morning startup"
    $ENABLE_CLAUDE && echo "    /new <brain dump>          # Triage input"
    $ENABLE_CLAUDE && echo "    /debrief                  # End-of-session log"
    echo ""
    echo -e "  ${BOLD}Available commands:${NC}"
    echo "    /today        Morning startup planner"
    echo "    /new          Universal brain-dump triage"
    echo "    /tldr         Summarize session or file"
    echo "    /debrief      End-of-session ritual"
    echo "    /vault-audit  Vault health check"
    echo "    /vault-align  Re-sync agent configs"
    echo "    /reflect      AI self-review"
    echo "    /ingest       Import external content"
    echo ""
}
